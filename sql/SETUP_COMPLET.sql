...
-- ────────────────────────────────────────────────────────────
-- ÉTAPE 9 : MENU KWETU (nyama choma, cuisine congolaise, buffet weekend)
INSERT INTO public.categories (nom, description, emoji, ordre, actif) VALUES
('Grillades Nyama Choma','Viandes et poissons grilles au feu','🔥',1,true),
('Plats Congolais','Cuisine traditionnelle congolaise','🇨🇩',2,true),
('Buffet Weekend','Buffet special weekend - formule complete','🍽️',3,true),
('Accompagnements','Riz, fufu, plantain, kwanga','🍚',4,true),
('Boissons','Bieres, jus, boissons fraiches','🥤',5,true)
ON CONFLICT DO NOTHING;

INSERT INTO public.produits (nom, description, prix, categorie_id, disponible, ordre) VALUES
('Nyama Choma Boeuf','Viande de boeuf grillee au feu de bois, epices',15.00,(SELECT id FROM categories WHERE nom='Grillades Nyama Choma'),true,1),
('Nyama Choma Chevre','Chevre grillee marinee aux epices africaines',16.00,(SELECT id FROM categories WHERE nom='Grillades Nyama Choma'),true,2),
('Poulet Grille Entier','Poulet entier marine, braise lentement',14.00,(SELECT id FROM categories WHERE nom='Grillades Nyama Choma'),true,3),
('Cotes de Porc Grillees','Grillees, citron, piment oiseau',13.00,(SELECT id FROM categories WHERE nom='Grillades Nyama Choma'),true,4),
('Poisson Braise','Tilapia ou capitaine entier, epices locales',12.00,(SELECT id FROM categories WHERE nom='Grillades Nyama Choma'),true,5),
...
('Liboke de Poisson','Poisson cuit en feuilles de bananier, epices',14.00,(SELECT id FROM categories WHERE nom='Plats Congolais'),true,3),
('Makayabu','Morue salee preparee a la congolaise',11.00,(SELECT id FROM categories WHERE nom='Plats Congolais'),true,4),
('Fufu au Gombo','Fufu de manioc, sauce gombo poisson',8.00,(SELECT id FROM categories WHERE nom='Plats Congolais'),true,5),
('Buffet Weekend Complet','Nyama choma + plat congolais + accompagnement + boisson',22.00,(SELECT id FROM categories WHERE nom='Buffet Weekend'),true,1),
('Buffet Famille','Grand assortiment pour 4-6 personnes',70.00,(SELECT id FROM categories WHERE nom='Buffet Weekend'),true,2),
('Riz Blanc',NULL,3.00,(SELECT id FROM categories WHERE nom='Accompagnements'),true,1),
('Fufu de Manioc',NULL,3.00,(SELECT id FROM categories WHERE nom='Accompagnements'),true,2),
('Plantain Frit',NULL,3.00,(SELECT id FROM categories WHERE nom='Accompagnements'),true,3),
('Kwanga','Pain de manioc',2.00,(SELECT id FROM categories WHERE nom='Accompagnements'),true,4),
('Biere Primus 65cl',NULL,4.00,(SELECT id FROM categories WHERE nom='Boissons'),true,1),
('Biere Turbo King','65cl',4.00,(SELECT id FROM categories WHERE nom='Boissons'),true,2),
('Jus Frais','Mangue, passion, ananas',4.00,(SELECT id FROM categories WHERE nom='Boissons'),true,3),
('Eau Minerale','75cl',2.00,(SELECT id FROM categories WHERE nom='Boissons'),true,4),
('Soda','Coca, Fanta ou Sprite 33cl',2.50,(SELECT id FROM categories WHERE nom='Boissons'),true,5)
ON CONFLICT DO NOTHING;
